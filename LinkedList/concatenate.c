...
int split(ListNode *head, ListNode **ptrEvenList, ListNode **ptrOddList)
{
	...
	while(temp!=NULL){
		if(temp->num%2==0){
			insertNode(ptrEvenList, i, temp->num);
			i++;
			temp = temp->next;
		}
		else{
			insertNode(ptrOddList, j, temp->num);
			j++;
			temp=temp->next;
		}
	}
...
int duplicateReverse(ListNode *head, ListNode **ptrNewHead){
	...
	while(temp!=NULL){
		if(insertNode(ptrNewHead, 0, temp->num)!= -1)
		temp = temp->next;
		else
		return -1;
	}
...
int moveMaxToFront(ListNode **ptrHead){
	...
	int max = (*ptrHead)->num;
	ListNode *pre;
	ListNode *maxpt=*ptrHead;

	if(*ptrHead==NULL|| temp==NULL)
	return -1;
